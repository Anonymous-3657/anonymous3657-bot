...
async def ensure_indexes():
    ...
    await db.announcements.create_index([("status", 1), ("publish_date", -1)])
    await db.announcements.create_index("is_deleted")
    await db.announcements.create_index("created_at")
    await db.platform_settings.create_index("_key", unique=True)
    await db.pdf_documents.create_index("downloads")
    await db.notifications.create_index([("user_id", 1), ("created_at", -1)])
    await db.notifications.create_index([("user_id", 1), ("read", 1)])
    await db.ratings.create_index([("target_type", 1), ("target_id", 1)])
    await db.ratings.create_index([("user_id", 1), ("target_type", 1), ("target_id", 1)], unique=True)
